...
async function hasMutualPeaceDiplomacy(
  db: DatabaseClient,
  cycleId: string,
  firstUserId: string,
  secondUserId: string
  ...
  const relations = await db.diplomacyRelation.findMany({
    where: {
      cycleId,
  ...
  const forward = relations.some(
    (relation) =>
      relation.fromUserId === firstUserId &&
      relation.toUserId === secondUserId
  );
  const reverse = relations.some(
    (relation) =>
      relation.fromUserId === secondUserId &&
      relation.toUserId === firstUserId
  );
...
export async function setFortressAction({
  ...
  return db.$transaction(async (tx) => {
    ...
    const activeDefenseOnTarget = await tx.attackUnit.findFirst({
      where: {
        attackerFortressId: fortress.id,
        missionKind: AttackUnitMissionKind.CASTLE_DEFENSE,
        defenseTargetFortressId: target.id,
        cancelledAt: null,
        defenseReleasedAt: null,
      },
      select: {
        id: true,
      },
    ...
    if (activeDefenseOnTarget) {
      throw new GameError(
        "You cannot attack a castle while your units are assigned to defend it."
      );
    }
...
export async function setDiplomacyStatus({
  userId,
  targetUserId,
  status,
  now = new Date(),
  ...
  userId: string;
  targetUserId: string;
  status: DiplomacyStatus | string;
  now?: Date;
  db?: PrismaClient;
}) {
  if (userId === targetUserId) {
    throw new GameError("You cannot set diplomacy with yourself.");
  }
  ...
  return db.$transaction(async (tx) => {
    ...
    if (!cycle || !isGameplayWindowOpen(cycle, now)) {
      throw new GameError("Diplomacy can only be updated during gameplay.");
    }

    const [sourceFortress, targetFortress] = await Promise.all([
    ...
    if (!sourceFortress || sourceFortress.isNpc) {
      throw new GameError("You are not participating in the active cycle.");
    }

    if (!targetFortress || targetFortress.isNpc) {
      throw new GameError(
        "That player is not participating in the active cycle."
      );
    }

    return tx.diplomacyRelation.upsert({
      where: {
        cycleId_fromUserId_toUserId: {
          cycleId: cycle.id,
          fromUserId: userId,
...
export async function deployCastleDefense({
  userId,
  targetFortressId,
  armyAmount,
  now = new Date(),
  ...
  return db.$transaction(async (tx) => {
    ...
    if (!cycle || !isGameplayWindowOpen(cycle, now)) {
      throw new GameError(
        "The current cycle is not accepting defense deployments."
      );
    }

    if (!Number.isInteger(armyAmount) || armyAmount <= 0) {
      throw new GameError("Send at least 1 army to defend.");
    }

    const sourceFortress = await tx.fortress.findUnique({
      where: {
        cycleId_ownerId: {
          cycleId: cycle.id,
          ownerId: userId,
    ...
    if (armyAmount > sourceFortress.army) {
      throw new GameError("You do not have enough idle army.");
    }

    const targetFortress = await tx.fortress.findFirst({
      where: {
        id: targetFortressId,
        cycleId: cycle.id,
        isNpc: false,
    ...
    if (!targetFortress) {
      throw new GameError("That castle cannot receive defenders.");
    }

    if (targetFortress.id === sourceFortress.id) {
      throw new GameError("Use your own garrison directly at home castle.");
    }

    const allied = await hasMutualPeaceDiplomacy(
      tx,
      cycle.id,
      sourceFortress.ownerId,
      targetFortress.ownerId
    );

    if (!allied) {
      throw new GameError(
        "You can defend only castles with mutual PEACE diplomacy."
      );
    }

    const outboundAttackCount = await tx.attackUnit.count({
      where: {
        attackerFortressId: sourceFortress.id,
        resolvedAt: null,
        cancelledAt: null,
      },
    });
    const maxAttacks = getMaxSimultaneousAttacks(
      sourceFortress.level,
      sourceFortress.race
    );

    if (outboundAttackCount >= maxAttacks) {
      throw new GameError(
        `You have reached the maximum number of simultaneous deployments (${maxAttacks}).`
      );
    }

    const existingDefense = await tx.attackUnit.findFirst({
      where: {
        attackerFortressId: sourceFortress.id,
        missionKind: AttackUnitMissionKind.CASTLE_DEFENSE,
        defenseTargetFortressId: targetFortress.id,
        cancelledAt: null,
        defenseReleasedAt: null,
      },
      select: {
    ...
    if (existingDefense) {
      throw new GameError(
        "You already have an active defense deployment for that castle."
      );
    }
    ...
    if (!launchedUnit) {
      throw new GameError(
        "That defense deployment would arrive after the cycle ends."
      );
    }

    return tx.attackUnit.update({
      where: {
        id: launchedUnit.id,
      },
      data: {
...
export async function recallStationedDefense({
  userId,
  attackUnitId,
  now = new Date(),
  db = prisma,
  ...
  return db.$transaction(async (tx) => {
    ...
    const deployedUnit = await tx.attackUnit.findUnique({
      where: {
        id: attackUnitId,
      },
      select: {
    ...
    if (
      !deployedUnit ||
      deployedUnit.cycleId !== cycle.id ||
      deployedUnit.attackerFortress.ownerId !== userId
    ) {
      throw new GameError("That defense deployment is not available.");
    }

    if (deployedUnit.missionKind !== AttackUnitMissionKind.CASTLE_DEFENSE) {
      throw new GameError("That unit is not a stationed defense deployment.");
    }

    if (!deployedUnit.defenseStationedAt || deployedUnit.defenseReleasedAt) {
      throw new GameError("That defense deployment is not currently stationed.");
    }

    if (
      deployedUnit.defenseTargetFortress &&
      deployedUnit.defenseTargetFortress.fortressKind === FortressKind.PLAYER
    ) {
      await tx.fortress.update({
        where: {
          id: deployedUnit.defenseTargetFortress.id,
        },
        data: {
          army: {
            decrement: Math.min(
              deployedUnit.defenseTargetFortress.army,
              deployedUnit.armyAmount
            ),
          },
        },
      });
    ...
    await tx.fortress.update({
      where: {
        id: deployedUnit.attackerFortress.id,
      },
      data: {
        army: {
          increment: deployedUnit.armyAmount,
        },
      },
    });
    ...
    return tx.attackUnit.update({
      ...
      data: {
        defenseReleasedAt: now,
        recalledAt: now,
        attackerReturned: deployedUnit.armyAmount,
      },
    });
  });
}
